...
// TestGenerationServiceClient is the client API for TestGenerationService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type TestGenerationServiceClient interface {
	GenerateTests(ctx context.Context, opts ...grpc.CallOption) (TestGenerationService_GenerateTestsClient, error)
}

type testGenerationServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewTestGenerationServiceClient(cc grpc.ClientConnInterface) TestGenerationServiceClient {
	return &testGenerationServiceClient{cc}
}

func (c *testGenerationServiceClient) GenerateTests(ctx context.Context, opts ...grpc.CallOption) (TestGenerationService_GenerateTestsClient, error) {
	stream, err := c.cc.NewStream(ctx, &TestGenerationService_ServiceDesc.Streams[0], "/services.TestGenerationService/GenerateTests", opts...)
	if err != nil {
		return nil, err
	}
	x := &testGenerationServiceGenerateTestsClient{stream}
	return x, nil
}

type TestGenerationService_GenerateTestsClient interface {
	Send(*KeployToServer) error
	Recv() (*ServerToKeploy, error)
	grpc.ClientStream
}

type testGenerationServiceGenerateTestsClient struct {
	grpc.ClientStream
}

func (x *testGenerationServiceGenerateTestsClient) Send(m *KeployToServer) error {
	return x.ClientStream.SendMsg(m)
}

func (x *testGenerationServiceGenerateTestsClient) Recv() (*ServerToKeploy, error) {
	m := new(ServerToKeploy)
...
// TestGenerationServiceServer is the server API for TestGenerationService service.
// All implementations must embed UnimplementedTestGenerationServiceServer
// for forward compatibility
type TestGenerationServiceServer interface {
	GenerateTests(TestGenerationService_GenerateTestsServer) error
	mustEmbedUnimplementedTestGenerationServiceServer()
}

// UnimplementedTestGenerationServiceServer must be embedded to have forward compatible implementations.
type UnimplementedTestGenerationServiceServer struct {
}

func (UnimplementedTestGenerationServiceServer) GenerateTests(TestGenerationService_GenerateTestsServer) error {
	return status.Errorf(codes.Unimplemented, "method GenerateTests not implemented")
}
func (UnimplementedTestGenerationServiceServer) mustEmbedUnimplementedTestGenerationServiceServer() {}

// UnsafeTestGenerationServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to TestGenerationServiceServer will
// result in compilation errors.
type UnsafeTestGenerationServiceServer interface {
	mustEmbedUnimplementedTestGenerationServiceServer()
}

func RegisterTestGenerationServiceServer(s grpc.ServiceRegistrar, srv TestGenerationServiceServer) {
	s.RegisterService(&TestGenerationService_ServiceDesc, srv)
}

func _TestGenerationService_GenerateTests_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TestGenerationServiceServer).GenerateTests(&testGenerationServiceGenerateTestsServer{stream})
}

type TestGenerationService_GenerateTestsServer interface {
	Send(*ServerToKeploy) error
	Recv() (*KeployToServer, error)
	grpc.ServerStream
}

type testGenerationServiceGenerateTestsServer struct {
	grpc.ServerStream
}

func (x *testGenerationServiceGenerateTestsServer) Send(m *ServerToKeploy) error {
	return x.ServerStream.SendMsg(m)
}

func (x *testGenerationServiceGenerateTestsServer) Recv() (*KeployToServer, error) {
	m := new(KeployToServer)
...
// TestGenerationService_ServiceDesc is the grpc.ServiceDesc for TestGenerationService service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var TestGenerationService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "services.TestGenerationService",
	HandlerType: (*TestGenerationServiceServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "GenerateTests",
			Handler:       _TestGenerationService_GenerateTests_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
