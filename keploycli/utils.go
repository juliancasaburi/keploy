...
// testInput is a struct to take the parameter input for startTest function
...
func fetch(t testInput) ([]models.TestCase, error) {
	...
	pageSize := 25
	for i := 0; ; i += pageSize {
		res, err := t.kServices.TestcaseSrv.GetAll(t.ctx, graph.DEFAULT_COMPANY, t.appId, &i, &pageSize, t.tcsPath, t.mockPath, "Hi")
		if err != nil {
			return nil, err
		}
		tcs = append(tcs, res...)
...
// startTest runs the recorded tests
func startTest(t testInput) {
...
// check asserts the responses of actual call with the recorded testcase data.
...
// simulate makes the http call to capture the actual responses for assertion
